...
@php
    $classes = 'relative min-h-14 mx-auto w-full bg-white ring-1 ring-e2eaeb rounded-[28px] flex divide-y';
@endphp
<div
    wire:ignore
    x-data="{
        ...
        <div {{ $attributes->merge(['class' => $classes]) }} :class="open && @json($inline) ? 'pb-5' : ''">
            <div :class="open ? 'h-auto' : 'h-14 overflow-hidden'" class="flex-1">
                <x-datepicker-input x-ref="picker" :label="$label"
                                    class="relative pl-12 min-h-14 mx-auto w-full text-sm bg-inherit border-none rounded-[30px] flex flex-col justify-center divide-y px-4 placeholder:text-0d171a focus:ring-0">
                    @isset($title)
                        <x-slot:title>
                            {{ $title }}
                        </x-slot:title>
